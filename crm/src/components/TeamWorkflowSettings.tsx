...
export default function TeamWorkflowSettings() {
  ...
  return <section className="card" aria-label="Managers and coverage">
    ...
    <p className="muted small">Salespeople own prospect communication, submissions, renewals and client service. Sales managers oversee all of this work. Unhandled work reaches the agency owner.</p>
    {(error || resource.error) && <p className="error-text" role="alert">{error || resource.error}</p>}
    {!draft ? <><div className="form-grid"><p><strong>Agency owner</strong><br />{name(routing.ownerId)}</p></div>
      <div className="table-wrap"><table><thead><tr><th>Teammate</th><th>Sales manager</th><th>Coverage</th></tr></thead><tbody>{team.filter(m => m.salesperson).map(m => { const r = routing.members.find(r => r.userId === m.userId); return <tr key={m.userId}><td>{m.name}</td><td>{m.salesperson ? name(r?.salesManagerId) : "—"}</td><td>{r?.away ? `Away · ${name(r.coverId)} covering` : "Available"}</td></tr>; })}</tbody></table></div>
      <p className="muted small">Reports arrive at 9 a.m. Eastern. {routing.reportChannelId ? "Internal reporting channel saved." : "An internal reporting channel still needs to be connected."}</p></> : <form onSubmit={async e => {
        ...
        <div className="form-grid">{([
          ["ownerId", "Agency owner"], ["intakeOwnerId", "Unmatched activity goes to"], ["integrationOwnerId", "Integration problems go to"],
        ] as const).map(([key, label]) => <label className="field" key={key}>{label}<select value={draft[key] ?? ""} onChange={e => setDraft(d => d && ({ ...d, [key]: e.target.value || undefined }))}><option value="">Choose teammate</option>{options()}</select></label>)}</div>
        <div className="table-wrap"><table><thead><tr><th>Teammate</th><th>Manager eligibility</th><th>Sales manager</th><th>Temporary cover</th></tr></thead><tbody>{team.map(m => {
          ...
          return <tr key={m.userId}><td>{m.name}</td><td><label><input type="checkbox" checked={!!r?.salesManager} onChange={e => updateMember(m.userId, { salesManager: e.target.checked })} /> Sales manager</label></td>
