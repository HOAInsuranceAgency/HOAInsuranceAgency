...
describe("independent app deployment", () => {
  it("rejects the old schema and validates the additive schema without executing intake", async () => {
    ...
    const compatible = await graphql({ schema: schema(", submissionId: String, retryProof: String, answerSnapshot: String, attribution: String, propertyKind: String, grossSquareFeet: String, replacementValue: String"), source: probe, rootValue: { submitWebLead: send } });
