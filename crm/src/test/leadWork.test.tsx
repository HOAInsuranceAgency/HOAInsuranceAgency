...
describe("staff follow-up", () => {
  it("has three work views and last contact without a redundant responsibility filter", async () => {
    ...
    expect(screen.queryByRole("combobox", { name: "Responsibility" })).not.toBeInTheDocument();
    expect(screen.queryByText("Deal champion")).not.toBeInTheDocument();
  ...
  it("drops a late pagination response after the work view changes", async () => {
    let finish!: (v: unknown) => void;
    h.request.mockImplementation((_op: string, input: { kind: string; nextToken?: string; view?: string }) => {
      if (input.kind !== "TASK") return Promise.resolve({ items: [] });
      if (input.nextToken) return new Promise(resolve => { finish = resolve; });
      return Promise.resolve(input.view === "Upcoming" ? { items: [{ ...future, id: "upcoming", name: "Upcoming lead" }] } : { items: [future], nextToken: "more" });
    });
    page(); fireEvent.click(await screen.findByRole("button", { name: "Load more" }));
    fireEvent.change(screen.getByRole("combobox", { name: "View" }), { target: { value: "Upcoming" } });
    await screen.findByText("Upcoming lead");
    await act(async () => finish({ items: [{ ...future, id: "late", name: "Wrong older page" }] }));
    expect(screen.queryByText("Wrong older page")).toBeNull(); expect(screen.getByText("Upcoming lead")).toBeVisible();
  });
});
