...
export const handler: Schema["submitWebLead"]["functionHandler"] = async event => {
  ...
  if ("readinessContract" in event.arguments) {
    if (event.arguments.readinessContract !== 1) return { ready: false, contractVersion: 1 };
    if (!["COMMUNICATION_TABLE", "ACCOUNT_TABLE", "CONTACT_TABLE", "PRIOR_CARRIER_TABLE", "LEAD_REPLY_TABLE", "USER_POOL_ID"].every(key => process.env[key])) return { ready: false, contractVersion: 1 };
    try { await get("config"); return { ready: true, contractVersion: 1 }; }
    catch { return { ready: false, contractVersion: 1 }; }
  }
  ...
  const account = { stage: "LEAD", type: isAccountType(args.type) ? args.type : DEFAULT_ACCOUNT_TYPE, name,
