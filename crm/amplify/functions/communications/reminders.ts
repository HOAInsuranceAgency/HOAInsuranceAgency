...
export async function migrateReminderSchedules() {
  ...
  const key = `migration:morning-reminders:v1:${hash(JSON.stringify([...holidays].sort()))}`;
  ...
  for (const candidate of page.items) {
    ...
    if (data.reminderAt === task.data.reminderAt && data.escalationAt === task.data.escalationAt) continue;
