...
export function historyDue(job?: Row<HistoryJob>, now = Date.now()) {
  ...
  if (!job.data.draftAware) return true;
...
export function restartHistory(id: string, conversationId: string, accountId?: string, previous?: Row<HistoryJob>) {
  return row<HistoryJob>("CONVERSATION_BACKFILL", id, { conversationId, attempts: 0, draftAware: true }, { accountId, previous, dueAt: new Date().toISOString() });
}
