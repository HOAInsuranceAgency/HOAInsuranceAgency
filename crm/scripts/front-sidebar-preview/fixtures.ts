...
const kinds: Record<string, LeadTask['kind']> = {first:'FIRST_CONTACT',callback:'CALLBACK',quote:'QUOTE_PRESENTATION',carrier:'CARRIER',renewal:'QUOTE_TARGET',service:'SERVICE',annual:'ANNUAL_RETURN',blocked:'SUBMISSION',batch:'SUBMISSION'};
const carrier = ['carrier','renewal','blocked','batch'].includes(scenario);
...
const context: WorkflowContext = { workflow:wf,team,actorId:role,trackingHealthy:scenario!=='gap',frontContext:{conversationId:'cnv_example',assigneeId:`tea_${bound||carrier?'champ':'sales'}`,routing:bound||carrier?'CHAMPION':'SALESPERSON',purpose:carrier?'CARRIER':'PROSPECT',context:task.context,policyId:task.policyId}, tasks:scenario==='healthy'?[]:[task],issues:[],communications:[{id:'comm_example',accountId:'example',conversationId:'cnv_example',provider:'front',providerId:'msg_example',channel:'EMAIL',direction:'INBOUND',at:'2026-09-10T14:00:00.000Z',from:'jane@example.test',to:['sales@example.test'],subject:scenario==='service'?'Certificate request':'Insurance review',text:scenario==='service'?'Please send our certificate of insurance to the property manager.':'Could you call me about our upcoming insurance renewal?',status:'RECEIVED',classification:'SUBSTANTIVE',resolved:scenario==='healthy',version:1}] };
if (scenario === 'blocked') Object.assign(task, { title: 'Submit to Example Mutual', sourceIds: [], milestone: true, term: '2026-10-01', blocker: { reason: 'Licensing pending', detail: 'Florida appointment is still pending.', ownerId: 'marketing', reviewAt: previewNow, recordedAt: '2026-09-11T19:00:00Z', recordedBy: 'champ' } });
if (scenario === 'batch') context.tasks = [
  ...Array.from({length: 4}, (_,i) => scheduleReminders({ ...task, id: `market_${i}`, title: `Submit to Example Market ${i + 1}`, kind: 'SUBMISSION', sourceIds: [], milestone: true, term: '2026-09-20', dueAt: '2026-09-10T21:00:00.000Z' })),
  scheduleReminders({ ...task, id: 'client_reply', title: 'Respond to client question', kind: 'RESPONSE', role: 'SALESPERSON', domain: 'CLIENT', context: 'LEAD', dueAt: '2026-09-10T21:00:00.000Z' }),
];
...
export async function communicationRequest<T=Record<string,unknown>>(op:string,input?:Record<string,unknown>,_write?:boolean):Promise<T> {
  ...
  else if(op==='updateBlocker') { task.blocker = input?.action === 'CLEAR' ? undefined : { reason: String(input?.reason || task.blocker?.reason || 'Licensing pending'), ownerId: String(input?.ownerId || task.blocker?.ownerId || 'marketing'), detail: String(input?.detail ?? task.blocker?.detail ?? ''), reviewAt: '2026-09-15T13:00:00.000Z', recordedAt: previewNow, recordedBy: role }; task.version++; result={}; notice('Blocker updated in this preview only. The original deadline and client updates are unchanged.'); }
