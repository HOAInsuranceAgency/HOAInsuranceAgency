...
export const probe = `mutation IntakeContract {
  submitWebLead(name: "", submissionId: "contract-check-no-write", retryProof: "contract-check-no-write-contract-check-no-write", answerSnapshot: "{}", attribution: "{}", propertyKind: "", grossSquareFeet: "", replacementValue: "") @skip(if: true)
}`;
export async function contractReady(url, key, request = fetch) {
  ...
  const state = (await read("query IntakeReadiness { leadIntakeReady(readinessContract: 3) }"))?.leadIntakeReady;
  const ready = typeof state === "string" ? JSON.parse(state) : state;
  return ready?.ready === true && ready.contractVersion >= 3;
}
