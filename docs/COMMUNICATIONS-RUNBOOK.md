...
- The main and individual business lines are monitored. New prospect text drafts use the shared **(508) 233-2261** channel and require a person to send in Front. Sent substantive replies and completed connected calls satisfy the matching contact reminder automatically; they do not prove document receipt or a deal outcome.
- Unknown callers/texts enter triage; a phone-number match alone never chooses an HOA. Staff can link one activity, combine selected requests with the earliest deadline, while the actual call/text supplies its history and automatic next follow-up. Carrier requests remain separate from prospect response work.
