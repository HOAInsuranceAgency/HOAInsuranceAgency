...
The hosting redirect adds a trailing slash to `/front-sidebar`; both authenticated and sign-in routing now recognize either form. Redeploy both staging apps with these corrections and verify their successful hosting steps, the deployed sidebar and response headers. Provider traffic acceptance remains pending Front/Dialpad credentials, test channels and designated recipients; infrastructure deployment alone does not complete that soak.
