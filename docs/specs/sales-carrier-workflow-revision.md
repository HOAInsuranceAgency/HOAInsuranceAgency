...
**September 25, 2026 update:** Scheduled reminders stay in CRM notifications and daily email reports. They no longer reopen/unsnooze Front conversations or add daily comments; queued deliveries from that retired feature are suppressed. This supersedes earlier Front resurfacing requirements. New inbound activity retains its normal behavior.
...
**Example:** A request arrives Thursday after business hours. Its response deadline is Friday at 5 p.m. Friday's CRM reminder arrives at 9 a.m.; it does not change the Front conversation status at either time. If unresolved, it appears in the manager's Monday 9 a.m. escalation report, assuming no holiday.
...
The morning report and the existing work list show the same current obligation. A daily report must not create a fresh task on every iteration. Scheduled reminders update CRM notifications and reports only: do not reopen, unsnooze or post reminder comments to Front conversations. Subsequent days retain the same task/card and the existing escalation schedule, regardless of whether someone archived or snoozed the linked conversation.
...
| Front is archived/snoozed while a task is pending | Deadline is unchanged; due work stays visible in CRM notifications and reports; scheduled reminders do not change Front status or post comments; automatic cleanup never hides a current unanswered request |
